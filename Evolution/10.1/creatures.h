...
public:
    ...
    //virtual void eat() = 0; // закомментировано для 4 задания (не будет работать когда необходимо создать vector<generic> т.к.  данном случае generic - абстрактный класс)

    // когда иллюстрируется полиморфное поведение , то этот метод можно закомментировать и раcкоментировть чисто виртуальный, тогда generic станет абстрактным классом
    virtual void eat() {
        std::cout << "\033[35m";
